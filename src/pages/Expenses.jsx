  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h2 className="text-2xl font-extrabold text-slate-900 tracking-tight">Shop Expenses Register</h2>
          <p className="text-xs text-slate-500 mt-1">Track operational shop costs (Rent, Electricity, Salary, Transport, Internet)</p>
        </div>
        <Button onClick={() => setIsModalOpen(true)} icon={Plus} className="bg-brand-600 hover:bg-brand-500 text-white font-bold">
          + Record New Expense
        </Button>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
        <Card className="p-4 bg-gradient-to-br from-slate-900 to-slate-800 text-white">
          ...
          <div className="text-3xl font-black text-rose-400 font-mono mt-1">₹{totalExpenseAmount.toLocaleString('en-IN')}</div>
              ...
              {loading ? (
                ...
                expenses.map((e) => (
                  <tr key={e.id} className="hover:bg-slate-50 transition-colors">
                    <td className="p-4 font-medium text-slate-600 text-xs">{e.expense_date}</td>
                    <td className="p-4 font-bold text-slate-900">
                      <Badge variant="amber">{e.category}</Badge>
                    </td>
                    <td className="p-4 text-slate-700 text-xs">{e.description || e.notes || '—'}</td>
                    <td className="p-4 text-center text-xs text-slate-600">{e.payment_mode}</td>
                    <td className="p-4 text-right font-extrabold text-rose-600">₹{parseFloat(e.amount).toFixed(2)}</td>
      ...
      <Modal isOpen={isModalOpen} onClose={() => setIsModalOpen(false)} title="+ Record New Shop Expense">
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="flex justify-end gap-3 pt-3 border-t border-slate-100">
            ...
            <Button type="submit" className="bg-brand-600 text-white font-bold">Save Expense</Button>
          </div>
        </form>
      </Modal>
    </div>
