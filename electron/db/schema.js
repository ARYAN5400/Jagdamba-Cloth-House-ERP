...
export async function initializeDatabaseSchema() {
  ...
  // Seed standard fabric/suit categories if none exist
  ...
  // 4. Products Table (NO DEMO PRODUCTS SEEDED!)
  await run(`
    CREATE TABLE IF NOT EXISTS products (
      ...
      colour TEXT,
      size TEXT,
      unit_type TEXT NOT NULL DEFAULT 'piece', -- 'piece', 'meter', 'other'
  ...
  // Ensure default Walk-in / Cash Customer exists
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS purchases (
      ...
      payment_mode TEXT DEFAULT 'Cash', -- 'Cash', 'Bank Transfer', 'UPI', 'Credit'
  ...
  await run(`
    CREATE TABLE IF NOT EXISTS expenses (
      ...
      category TEXT NOT NULL, -- 'Rent', 'Electricity', 'Salary', 'Transport', 'Internet', 'Maintenance', 'Other'
      description TEXT,
  ...
  // 14. Automatic Schema Migrations (Idempotent Column Checks)
  await migrateDatabaseColumns();

  console.log('[Database Schema] Database initialization & schema migration complete.');
}

async function migrateDatabaseColumns() {
  console.log('[Database Migration] Checking table schemas for missing columns...');
  ...
  try {
    ...
    for (const col of columnsToEnsure) {
      if (!existingNames.has(col.name.toLowerCase())) {
        console.log(`[Database Migration] Adding missing column '${col.name}' to table '${tableName}'...`);
